...
def parse_ctc_labels_from_root(root):
    letter2index = {}
    index2letter = {}

    with open('data/letters.lst', 'r') as f:
    ...
    for file in pathlib.Path(root).rglob("*.txt"):
...
def find_seqs(dir_name, extension='.flac'):
    sequences = []
    for file in pathlib.Path(dir_name).rglob('*' + extension):
