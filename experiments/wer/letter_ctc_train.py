...
import jiwer
from letter_ctc import LetterClassifier, CTCLetterCriterion, cut_data
from simple_dataset import SingleSequenceDataset, parse_ctc_labels_from_root, find_seqs
from wl_decoder import WlDecoder
...
def eval_wer(loader,
    ...
    for index, data in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            predictions = criterion.module.letter_classifier(
...
def main(args):
    ...
    with open('data/letters.lst') as f:
        n_chars = len(f.readlines())

    feature_maker, hiddenGar, _ = loadModel([args.path_checkpoint.strip()],
                                            loadStateDict=True)
    letter_classifier = LetterClassifier(
        feature_maker,
        hiddenGar, n_chars, p_dropout=args.p_dropout if hasattr(args, 'p_dropout') else 0.0)
    ...
    if args.path_train and args.path_val:
        ...
        char_labels_val, n_chars, _ = parse_ctc_labels_from_root(args.path_val)
