...
class SingleSequenceDataset(Dataset):
    def __init__(self,
        ...
        """
        self.seq_names = deepcopy(seq_names)
    ...
    def loadSeqs(self):
        ...
        to_load = [Path(self.path_database) / x for _, x in self.seqNames]
    ...
    def __getitem__(self, idx):
        ...
        outPhone[:sizePhone] = self.phoneLabels[
            offsetPhoneStart:offsetPhoneEnd]

        return outSeq, torch.tensor(
            [sizeSeq],
            dtype=torch.long), outPhone.long(), torch.tensor([sizePhone],
                                                             dtype=torch.long)
...
class CTCphone_criterion(torch.nn.Module):
    def __init__(self,
        ...
        self.dropout = torch.nn.Dropout2d(p=0.5,
                                          inplace=False) if dropout else None
        self.conv1 = torch.nn.LSTM(dimEncoder,
                                   dimEncoder,
                                   num_layers=1,
                                   batch_first=True)
        self.PhoneCriterionClassifier = torch.nn.Conv1d(dimEncoder,
                                                        nPhones + 1,
                                                        sizeKernel,
                                                        stride=sizeKernel // 2)
    ...
    def forward(self, cFeature, featureSize, label, labelSize):
        ...
        loss = self.lossCriterion(predictions, label, featureSize,
                                  labelSize).view(1, -1)
    ...
    for index, data in enumerate(val_loader):
        ...
        with torch.no_grad():
            ...
            predictions = torch.nn.functional.softmax(
                criterion.module.getPrediction(c_feature,
                                               sizeSeq), dim=2).cpu()
            phone = phone.cpu()
            sizeSeq = sizeSeq.cpu()
...
def run(train_loader, val_loader, model, criterion, optimizer,
        downsampling_factor, nEpochs, pathCheckpoint):

    print(f"Starting the training for {nEpochs} epochs")
    bestLoss = float('inf')

    for epoch in range(nEpochs):
        lossTrain = train_step(train_loader, model, criterion, optimizer,
                               downsampling_factor)
...
if __name__ == "__main__":
    ...
    parser_train.add_argument('pathDB',
                              type=str,
                              help='Path to the directory containing the '
                              'audio data / pre-computed features.')
    parser_train.add_argument('pathPhone',
                              type=str,
                              help='Path to the .txt file containing the '
                              'phone transcription.')
    parser_train.add_argument('pathCheckpoint',
                              type=str,
    ...
    parser_train.add_argument('--pathTrain',
                              default=None,
                              type=str,
                              help='Path to the .txt files containing the '
                              'list of the training sequences.')
    parser_train.add_argument('--pathVal',
                              default=None,
                              type=str,
                              help='Path to the .txt files containing the '
                              'list of the validation sequences.')
    parser_train.add_argument('--file_extension',
                              type=str,
                              default=".mp3",
    ...
    parser_train.add_argument('--beta1',
                              type=float,
                              default=0.9,
                              help='Value of beta1 for the Adam optimizer.')
    parser_train.add_argument('--beta2',
                              type=float,
                              default=0.999,
                              help='Value of beta2 for the Adam optimizer.')
    parser_train.add_argument('--epsilon',
                              type=float,
                              default=1e-08,
    ...
    val_loader = DataLoader(datasetVal,
    ...
    if args.command == 'train':
        ...
        datasetTrain = SingleSequenceDataset(args.pathDB,
                                             seqTrain,
                                             phoneLabels,
                                             inDim=args.in_dim)

        train_loader = DataLoader(datasetTrain,
                                  batch_size=args.batchSize,
                                  shuffle=True)
        ...
        perStep(val_loader, feature_maker, phone_criterion,
                downsampling_factor)
